...
function LanguageSelector({hasAccess} : {hasAccess : boolean})  {
  ...
  const handleLanguageSelect = (langId:string) => {
    if (!hasAccess && langId !== 'javascript' && langId !== 'typescript' && langId !== 'cpp' && langId!== 'python' && langId !== 'java') return;
    setLanguage(langId);
    setIsOpen(false);
  }
  return ( 
    <div className='relative' ref={dropdownRef}>
        ...
        className={`group relative flex items-center gap-3 px-4 py-2.5 bg-[#1e1e2e]/80 rounded-lg
          transition-all duration-200 border border-gray-800/50 hover:border-gray-700
          ${!hasAccess && language !== "javascript" && language != 'typescript' &&  language!= 'cpp' &&  language!= 'python' && language!= 'java' ? "opacity-50" : ""}`}
        >
          ...
          <div className='size-6 rounded-md bg-gray-800/50 p-0.5 group-hover:scale-110 transition-transform'>
            <Image src={currentLanguageObj.logoPath} alt='programming language logo'
            width={24} height={24}
            className='w-full h-full object-contain relative z-10' />
          </div>

          <span className='text-gray-200 min-w-[80px] text-left group-hover:text-white transition-colors'
          >{currentLanguageObj.label}</span>

          <ChevronDownIcon 
          className={`size-4 text-gray-400 transition-all group-hover:text-gray-300 duration-300
          ${isOpen ? "rotate-180" : ""}`}/>
        </motion.button>
          ...
          >
            ...
            <div className='max-h-[280px] overflow-y-auto overflow-x-hidden'>
                {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                  const isLocked = !hasAccess && lang.id !== 'javascript' && lang.id !== 'typescript' && lang.id !== 'cpp' && lang.id !== 'python' && lang.id !== 'java';
                  return (
                    <motion.div
                      key={lang.id}
                      initial={{ opacity: 0 }}
                    ...
                    >
                      ...
                      >
                        ...
                        <div className={`realtive size-8 rounded-lg p-1.5 group-hover:scale-110 transition-transform
