...
const RunButton = () => {
  ...
  const handleRun = async () => {
    await runCode();
    const resultExecution  = getExecutionResult();

    if(user && resultExecution) {
      // todo : save the result to the convex
        await saveExecution({
          language,
          code: resultExecution.code,
          output: resultExecution.output || undefined,
          error: resultExecution.error || undefined
        })
    }
  }
    ...
    >
      ...
      <div className='relative flex items-center gap-2.5'>
        ...
        <div className='relative flex items-center justify-center size-4'>
          <Play className='size-4 text-white/90 transition-transform group-hover:scale-110
