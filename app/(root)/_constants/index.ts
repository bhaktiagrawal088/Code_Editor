...
type LanguageConfig = Record<
  string,
  {
    ...
    logoPath: string;          // Path to the language logo (for UI purposes).
    pistonRuntime: {           // Specifies the runtime environment for executing the code (uses Piston API).
      language: string;        // Language name supported by the runtime.
      version: string;         // Version of the language runtime.
    };
    monacoLanguage: string;    // Monaco Editor's internal language ID (for syntax highlighting).
    defaultCode: string;       // Code snippet to display by default when this language is selected.
  }
